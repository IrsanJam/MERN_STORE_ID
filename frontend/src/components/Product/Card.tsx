...
const Card: FC<productDataType> = (props: productDataType) => {
  const { type, price, processor, cekProduk, allData, image, id, brand } =
    props;
  ...
  return (
    <>
      <div
        id={`card-${id}`}
        className="flex flex-col justify-center items-start shadow-md border-2   border-b-zinc-600 rounded-md md:p-5 p-3 font-Poppins gap-2"
      >
        <img
          id={`image-${id}`}
          ...
          alt="image"
          width={"200px"}
          className=" min-h-[8rem] max-h-[8rem]  rounded-md"
        />
        <span
          id={`type-${id}`}
          className="font-bold md:text-sm text-xs font-Poppins"
        >{`${brand} ${type}`}</span>
        ...
        <div className="flex justify-center items-center w-full md:gap-5 gap-1">
          <button
            onClick={cekProduk}
            id={`view-product-${id}`}
            className="w-[80%] lg:text-base text-xs py-2 bg-[#0396C7] text-white rounded-md"
          >
            Lihat Produk
          </button>

          <button
            onClick={() => addCart(allData, id)}
            id={`add-to-cart-${id}`}
            className={`md:w-[20%] w-[23%] bg-slate-300 md:py-2 py-[3px]  rounded-md flex justify-center items-center`}
          >
            <img
              src={keranjangIcon}
              className="w-10 p-1  md:w-6 md:p-[0.3px]"
            />
          </button>
        </div>
      </div>
